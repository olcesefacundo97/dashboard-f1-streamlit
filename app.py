...
st.set_page_config(layout="wide", page_title="Dashboard Interactivo - Fórmula 1", page_icon="🏎️")

# ========== FUNCIONES EXPORT ==========
def exportar_grafico(fig, nombre_archivo, texto_boton):
    buffer = io.BytesIO()
    fig.write_image(buffer, format="png")
    return st.download_button(texto_boton, buffer.getvalue(), nombre_archivo, "image/png")

def exportar_csv(df, nombre_archivo, texto_boton):
    return st.download_button(texto_boton, df.to_csv(index=False), nombre_archivo, "text/csv")
...
def obtener_clima_para_carreras(df_resultados):
    clima_por_carrera = {}
    for _, fila in df_resultados[['Circuito', 'Fecha', 'Lat', 'Lon']].drop_duplicates().iterrows():
    ...
    df_resultados["Clima"] = df_resultados.apply(
        lambda row: clima_por_carrera.get((row["Circuito"], row["Fecha"]), "Desconocido"),
        axis=1
    )
...
with st.sidebar:
    ...
    with st.spinner("Cargando datos..."):
        if f"df_{temporada}" not in st.session_state:
            ...
            if all(col in df.columns for col in ['Temporada', 'Ronda', 'Piloto']) and all(col in df_clasif.columns for col in ['Temporada', 'Ronda', 'Piloto']):
                df = df.merge(df_clasif, on=['Temporada', 'Ronda', 'Piloto'], how='left')
            else:
                st.error("❌ No se pudo hacer el merge: faltan columnas en los datos cargados.")
                st.stop()
    ...
    clima_seleccionado = st.multiselect("Filtrar por clima", climas_disponibles, default=climas_disponibles)
    ...
    escuderia = st.selectbox("Filtrar por escudería", ["Todos"] + escuderias_disponibles)
...
df_filtrado['Abandono'] = ~df_filtrado['Status'].str.contains("Finished", case=False, na=True)

tabs = st.tabs([
    "Mapa", "Rankings", "Comparaciones", "Predicción", "Métricas", "Correlación", "Radar", "Feedback", "Conclusiones"
])

# ========== TAB: MAPA ==========
with tabs[0]:
    st.title("🗺️ Mapa interactivo de circuitos")
    df_map = df_filtrado[['Lat', 'Lon', 'Circuito', 'Pais', 'Fecha']].dropna().rename(columns={'Lat': 'latitude', 'Lon': 'longitude'})
...
with tabs[1]:
    st.title("🏁 Rankings")
    ranking_pilotos = df_filtrado.groupby("Piloto")["Puntos"].sum().sort_values(ascending=False).reset_index()
    ranking_escuderias = df_filtrado.groupby("Escudería")["Puntos"].sum().sort_values(ascending=False).reset_index()
    col1, col2 = st.columns(2)
    with col1:
        st.subheader("Pilotos")
        st.dataframe(ranking_pilotos)
        exportar_csv(ranking_pilotos, "ranking_pilotos.csv", "Descargar ranking de pilotos")
    with col2:
        st.subheader("Escuderías")
        st.dataframe(ranking_escuderias)
        exportar_csv(ranking_escuderias, "ranking_escuderias.csv", "Descargar ranking de escuderías")

# ========== TAB: COMPARACIONES ==========
with tabs[2]:
    st.title("🆚 Comparaciones entre pilotos y escuderías")
    pilotos_sel = st.multiselect("Seleccioná pilotos", sorted(df_filtrado['Piloto'].unique()), default=sorted(df_filtrado['Piloto'].unique())[:2])
    if pilotos_sel:
        promedios = df_filtrado[df_filtrado['Piloto'].isin(pilotos_sel)].groupby('Piloto')['Puntos'].mean().reset_index()
        fig1 = px.bar(promedios, x='Piloto', y='Puntos', title="Puntos promedio por piloto", text='Puntos')
        ...
        fig2 = px.line(df_comp, x='Ronda', y='Acumulado', color='Piloto', title="Evolución de puntos acumulados")
        st.plotly_chart(fig2)
        clima_comparado = df_filtrado[df_filtrado['Piloto'].isin(pilotos_sel)].groupby(['Piloto', 'Clima'])['Puntos'].mean().reset_index()
        fig3 = px.bar(clima_comparado, x='Clima', y='Puntos', color='Piloto', barmode='group', title="Rendimiento por clima")
        st.plotly_chart(fig3)

    escuderias_sel = st.multiselect("Seleccioná escuderías", sorted(df_filtrado['Escudería'].unique()), default=sorted(df_filtrado['Escudería'].unique())[:2])
    if escuderias_sel:
        promedios_escu = df_filtrado[df_filtrado['Escudería'].isin(escuderias_sel)].groupby('Escudería')['Puntos'].mean().reset_index()
        fig4 = px.bar(promedios_escu, x='Escudería', y='Puntos', title="Puntos promedio por escudería", text='Puntos')
        ...
        fig5 = px.line(df_escu, x='Ronda', y='Acumulado', color='Escudería', title="Evolución de puntos acumulados por escudería")
        st.plotly_chart(fig5)
        clima_escu = df_filtrado[df_filtrado['Escudería'].isin(escuderias_sel)].groupby(['Escudería', 'Clima'])['Puntos'].mean().reset_index()
        fig6 = px.bar(clima_escu, x='Clima', y='Puntos', color='Escudería', barmode='group', title="Rendimiento por clima de escuderías")
        st.plotly_chart(fig6)

# ========== TAB: PREDICCIÓN ==========
with tabs[3]:
    st.title("🤖 Predicción de posición final")
    df_modelo = df_filtrado[['PosicionClasificacion', 'Posición', 'Clima', 'Escudería']].dropna()
    if not df_modelo.empty:
        X = df_modelo[['PosicionClasificacion', 'Clima', 'Escudería']]
        y = df_modelo['Posición']
        pre = ColumnTransformer([('cat', OneHotEncoder(drop='first'), ['Clima', 'Escudería'])], remainder='passthrough')
        modelo = Pipeline([('prep', pre), ('rf', RandomForestRegressor(n_estimators=100, random_state=42))])
        X_train, X_test, y_train, y_test = train_test_split(X, y, random_state=42)
        modelo.fit(X_train, y_train)
        y_pred = modelo.predict(X_test)
        mae = mean_absolute_error(y_test, y_pred)
        st.write(f"📏 Error absoluto medio del modelo: **{int(round(mae))} posiciones**")
        ...
        with col3:
            escu_in = st.selectbox("Escudería", sorted(df_filtrado['Escudería'].unique()))
        ...
        if st.button("Predecir posición final"):
            ...
            st.success(f"🔮 La posición final estimada para este piloto es: **{pos_entera}°**")
    else:
        st.info("No hay datos suficientes para entrenar el modelo con los filtros aplicados.")

# ========== TAB: MÉTRICAS ==========
with tabs[4]:
    st.title("📈 Métricas de desempeño")
    metricas = df_filtrado.groupby('Piloto').agg({
        'Victoria': 'sum',
        'Posición': 'mean',
        'Abandono': 'mean'
    ...
    st.dataframe(metricas.sort_values(by=['Carreras Ganadas', 'Promedio de Posición', '% Abandono'], ascending=[False, True, True]))
    st.subheader("Escuderías")
    metricas_escu = df_filtrado.groupby('Escudería').agg({
        'Victoria': 'sum',
        'Posición': 'mean',
        'Abandono': 'mean'
    }).reset_index().rename(columns={
    ...
    st.dataframe(metricas_escu.sort_values(by=['Carreras Ganadas', 'Promedio de Posición', '% Abandono'], ascending=[False, True, True]))
...
with tabs[5]:
    ...
    pilotos_sel_box = st.multiselect("Seleccioná pilotos para el boxplot", sorted(df_filtrado['Piloto'].unique()), default=sorted(df_filtrado['Piloto'].unique())[:5], key="boxplot_pilotos")
    if pilotos_sel_box:
        df_box = df_filtrado[df_filtrado['Piloto'].isin(pilotos_sel_box)]
        fig_box = px.box(df_box, x='Piloto', y='Posición', color='Clima', points='all', title="Distribución de posiciones finales por clima y piloto")
        st.plotly_chart(fig_box)

    st.subheader("Consistencia de pilotos (STD de posiciones finales, menor es mejor)")
    consistencia = df_filtrado.groupby('Piloto')['Posición'].std().reset_index().rename(columns={'Posición':'STD_Posición'})
    consistencia = consistencia.sort_values('STD_Posición')
    st.dataframe(consistencia)

    st.subheader("Consistencia de escuderías (STD de posiciones finales, menor es mejor)")
    consistencia_escu = df_filtrado.groupby('Escudería')['Posición'].std().reset_index().rename(columns={'Posición':'STD_Posición'})
    consistencia_escu = consistencia_escu.sort_values('STD_Posición')
    st.dataframe(consistencia_escu)
    fig_escu_std = px.bar(consistencia_escu, x="Escudería", y="STD_Posición", title="Consistencia de escuderías (menor es mejor)")
    ...
    pivot_clima = df_filtrado.pivot_table(index='Piloto', columns='Clima', values='Puntos', aggfunc='mean')
    if 'Seco' in pivot_clima.columns and 'Lluvia' in pivot_clima.columns:
        pivot_clima = pivot_clima[['Seco', 'Lluvia']].dropna()
        pivot_clima['Diferencia_Lluvia-Seco'] = pivot_clima['Lluvia'] - pivot_clima['Seco']
        st.dataframe(pivot_clima.sort_values('Diferencia_Lluvia-Seco'))
        fig_diff = px.bar(
            pivot_clima.reset_index(),
            x='Piloto', y='Diferencia_Lluvia-Seco',
            title="Diferencia de puntos promedio por piloto: Lluvia - Seco",
            color='Diferencia_Lluvia-Seco', color_continuous_scale="RdBu"
        )
        st.plotly_chart(fig_diff)
    else:
        st.info("No hay suficientes datos de ambos climas para comparar el rendimiento de los pilotos.")
...
with tabs[6]:
    ...
    radar_metricas = df_filtrado.groupby('Piloto').agg({
        'Puntos': 'mean',
        'Victoria': 'mean',
        'Abandono': 'mean'
    }).reset_index().rename(columns={
    ...
    pilotos_radar = st.multiselect("Seleccioná pilotos para el radar", radar_metricas['Piloto'].unique(), default=list(radar_metricas['Piloto'].unique())[:3])
    if pilotos_radar:
...
with tabs[7]:
    ...
    if st.button("Borrar feedback guardado"):
        try:
            if os.path.exists(ruta_pilotos): os.remove(ruta_pilotos)
            if os.path.exists(ruta_escuderias): os.remove(ruta_escuderias)
...
with tabs[8]:
    ...
    pivot_clima = df_filtrado.pivot_table(index='Piloto', columns='Clima', values='Puntos', aggfunc='mean')
    if 'Seco' in pivot_clima.columns and 'Lluvia' in pivot_clima.columns and not pivot_clima.empty:
        pivot_clima = pivot_clima[['Seco', 'Lluvia']].dropna()
        mayor = pivot_clima['Diferencia_Lluvia-Seco'] = pivot_clima['Lluvia'] - pivot_clima['Seco']
...
st.markdown("---")
st.markdown("Desarrollado por Emanuel Gallo y Facundo Olcese | Proyecto Integrador F1 | 2025")
